...
| **local mode** | Repo registered via `specd connect .`; code stays on the machine and specd holds no host credential. Output is a branch — pushed and opened as a real PR/MR where `gh`/`glab` is signed in on that machine (`knowledge/decisions/0020-local-mode-borrows-the-host-cli.md`), and a branch you diff where it is not. |
